...
Prerendering calls the same React component used by the browser. Pages are stored compressed to bound image size. HTML is shared by schema identity: `path`, legacy `linked`, and `trail` do not affect it, while `pointer`, `oneOf`, and `key` still select the documented schema. Canonical requests hydrate directly. On contextual visits, a synchronous startup guard prevents canonical links from being clicked while the application loads. The browser loads the original URL's page data from `/api/page`, then restores its heading, links, and circular-reference limits before enabling navigation. Failed context loads show recovery controls. Without JavaScript, schema documentation and canonical navigation remain available, but traversal history is not restored. Unknown resources return HTTP 404; malformed schema queries return HTTP 400. [ADR 0005](adr/0005-share-canonical-html-and-restore-traversal-in-the-browser.md) records the caching tradeoff.
...
Successful public HTML, JSON, crawler documents, redirects, and ordinary assets advertise a seven-day shared cache lifetime. Browsers revalidate those URLs, so a Cloudflare purge exposes updated content without requiring users to clear their browser cache. Hashed assets retain a one-year immutable lifetime. Health checks and errors are not cached. The Cloudflare proxy removes `path`, `linked`, and `trail` from documentation cache keys and origin requests, preserves schema selectors and complete API queries, and reports `X-Manifests-Cache: HIT`, `MISS`, or `BYPASS`. Deploy the browser context-restoration support before enabling this edge normalization, then purge existing entries. Roll back the edge normalization before reverting to a browser build without context restoration. The Spacelift deployment hook purges the zone after a successful apply. To bust the cache manually, run `python3 ../../cloudflare/workers/purge-manifests-cache.py` as a task on `manifests-production`, or use Cloudflare's zone-wide **Purge Everything** action. Publishing an image alone does not invalidate the cache.
